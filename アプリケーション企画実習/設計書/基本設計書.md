...
|is_birthday_estimated|boolean|notnull,defaulttrue|誕生日が推定かどうか|
...
│   ├─ HogotaroApplication.java       起動クラス
│   ├─ config/
│   │   ├─ SecurityConfig.java        ログインと権限の設定
│   │   └─ LoginUserDetailsService.java  ログインIDでstaffを探す
│   ├─ controller/
│   │   ├─ TopController.java         トップページ
│   │   ├─ KotaiController.java       個体
│   │   ├─ SatooyaController.java     里親
│   │   ├─ EventController.java       イベント
│   │   ├─ StaffController.java       スタッフ
│   │   └─ ErrorPageController.java   403,404,500のページ
│   ├─ service/                       各Controllerと同じ名前でXxxService.java
│   ├─ repository/                    各EntityにXxxRepository.java（interface）
│   ├─ entity/
│   │   ├─ Dantai.java  Staff.java  Kotai.java  Satooya.java  Event.java
│   │   ├─ UserType.java  EventType.java  Breed.java   マスタ
│   │   └─ Species.java  Sex.java  Gender.java  Status.java   enum
│   ├─ form/
│   │   └─ KotaiForm.java  SatooyaForm.java  EventForm.java  StaffForm.java   入力チェック用
│   ├─ exception/
│   │   ├─ NotFoundException.java     データが見つからないとき
│   │   └─ GlobalExceptionHandler.java  エラーページに飛ばす
│   └─ security/
│       └─ LoginUser.java             ログイン中の人の情報（id、団体id、権限）
├─ src/main/resources/
│   ├─ application.properties         DB接続とか
│   └─ static/
│       ├─ css/style.css
│       ├─ js/confirm.js              削除の確認ダイアログ
│       └─ img/placeholder-dog.png  placeholder-cat.png   写真がないとき用
├─ src/main/webapp/WEB-INF/views/
│   ├─ common/header.jspf  footer.jspf  messages.jspf   共通部品
│   ├─ login.jsp  top.jsp
│   ├─ error/403.jsp  404.jsp  500.jsp
│   ├─ kotai/list.jsp  detail.jsp  form.jsp     formは新規と編集で共用
│   ├─ satooya/list.jsp  detail.jsp  form.jsp
│   ├─ event/calendar.jsp  detail.jsp  form.jsp
│   └─ staff/list.jsp  detail.jsp  form.jsp
├─ uploads/                           個体の写真の保存先（gitには入れない）
└─ sql/
    ├─ ddl.sql                        テーブル作成
    ├─ data.sql                       初期データ（マスタ、デモ団体）
    └─ new_dantai.sql                 団体を追加するときのSQL
```
